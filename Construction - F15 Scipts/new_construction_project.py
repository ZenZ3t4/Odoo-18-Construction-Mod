...
#     2.5 Overhead Spent
# Changelog:
# 20250401 - Added f15_euro_value style to format European currency values
#            after creating the style, i've used the new style to print out correct values on excel cell
...
class ConstructionProject(models.Model):
    ...
    start_date = fields.Date(string="Schedule Data Inizio", tracking=True)
    end_date = fields.Date(string="Schedule Data Fine", tracking=True)
        ...
        for rec in self:
            ...
            if duplicate:
                raise ValidationError(_(
                    "Duplicate Tipi di Lavoro and sub Tipi di Lavoro pair found."))
...
class ConstructionExtraExpense(models.Model):
    ...
    note = fields.Char()
    project_id = fields.Many2one('tk.construction.project')
    qty = fields.Integer(string="Qta", default=1)
    vendor_id = fields.Many2one('res.partner')
...
class BoqBudget(models.Model):
    ...
    activity_id = fields.Many2one('job.type', string="Tipologia Lavorazione")
    sub_activity_ids = fields.Many2many(
        related="activity_id.sub_category_ids", string="Sub Activities")
    sub_activity_id = fields.Many2one('job.sub.category', string="Sottotipo lavorazione",
                                      domain="[('id','in',sub_activity_ids)]")
    qty = fields.Float(string="Qta", default=1.0)
    total_qty = fields.Float(
        string="Qta Totale.", compute="_compute_total_qty", store=True)
    length = fields.Float()
    width = fields.Float()
    height = fields.Float()
    is_use_measure = fields.Boolean(
    ...
    def _onchange_activity_id(self):
        """Empty Sub Type on Tipi di Lavoros"""
        for rec in self:
            if rec.activity_id:
